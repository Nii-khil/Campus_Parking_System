...
const PermitStatus = () => {
    ...
    try {
      ...
      if (response.ok) {
        setPermits((prevPermits) =>
          prevPermits.map((permit) =>
            permit.permit_id === id
              ? { ...permit, status: 'active', expiry_date: data.expiryDate }
              : permit
          )
        );
        setMessage('Permit renewed successfully');
  ...
  return (
    <div className="bg-gray-900 p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-semibold mb-6 text-white">Permit Status</h2>
        ...
        <table className="w-full border-collapse mt-4">
          <thead>
            <tr className="bg-gray-700">
              ...
              <th className="border border-gray-600 p-2 text-left text-white">Expiration Date</th>
              <th className="border border-gray-600 p-2 text-left text-white">Status</th>
              <th className="border border-gray-600 p-2 text-left text-white">Actions</th>
            </tr>
          </thead>
          <tbody>
            {permits.map((permit) => (
              <tr key={permit.permit_id} className="bg-gray-800 text-gray-200">
                <td className="border border-gray-600 p-2">{permit.permit_name}</td>
                <td className="border border-gray-600 p-2">{formatDate(permit.expiry_date)}</td>
